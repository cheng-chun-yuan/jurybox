...
-- Maps to CreateJudgeRequest from frontend (app/create-judge/page.tsx)
CREATE TABLE judges (
  id INT AUTO_INCREMENT PRIMARY KEY,

  -- Basic Information
  name VARCHAR(255) NOT NULL,                    -- Agent Name (e.g., Dr. Academic) [frontend: name]
  title VARCHAR(255),                            -- Professional Title (e.g., Research Specialist) [frontend: title]
  tagline TEXT,                                  -- JSON array of tags (e.g., ["Academic", "Creative", "Technical"]) [frontend: tagline (array)]
  description TEXT,                              -- Biography / Description [frontend: description]
  avatar VARCHAR(500),                           -- Agent Image URL or IPFS hash [frontend: avatar]
  theme_color VARCHAR(7) DEFAULT '#8B5CF6',      -- Theme Color (hex code: #8B5CF6, #06B6D4, #F59E0B) [frontend: themeColor]

  -- Specialties
  specialties TEXT,                              -- JSON array of specialties [frontend: specialties (array)]

  -- AI Model Configuration
  model_provider VARCHAR(50) DEFAULT 'OpenAI',   -- Model Provider (OpenAI, Anthropic, Groq, Ollama) [frontend: modelProvider]
  model_name VARCHAR(100) DEFAULT 'gpt-4',       -- Model Name (gpt-4, claude-3, etc.) [frontend: modelName]
  system_prompt TEXT NOT NULL,                   -- System Prompt for AI [frontend: systemPrompt]
  temperature DECIMAL(3, 2) DEFAULT 0.70,        -- Temperature (0.00 - 1.00) [frontend: temperature]

  -- Pricing & Performance
  price DECIMAL(10, 3) NOT NULL DEFAULT 0.050,   -- Price per judgment in HBAR [frontend: price]

  -- Payment Configuration (X402/A2A)
  wallet_address VARCHAR(255),                   -- Hedera account ID or EVM address to receive payments [frontend: walletAddress]
  payment_page_url VARCHAR(500),                 -- X402/A2A payment page URL (auto-generated by backend)

  rating DECIMAL(3, 2) DEFAULT 0.00,             -- Judge rating (0.00 - 5.00, auto-calculated)
  total_judgments INT DEFAULT 0,                 -- Total number of judgments completed (auto-incremented)

  -- IPFS Metadata (ERC-8004 standard)
  ipfs_cid VARCHAR(255),                         -- IPFS CID for agent metadata (auto-generated or provided) [frontend: ipfsCid (optional)]
